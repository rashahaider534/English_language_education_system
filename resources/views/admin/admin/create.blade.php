    ...
    <form method="POST" action="{{ route('admins.permission.store') }}" style="background:#EFFAFD; border:1.5px solid rgba(0,83,122,0.16); border-radius:22px; padding:26px; max-width:640px; box-shadow:0 18px 44px rgba(0,83,122,0.06);">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
            <div>
                ...
                <input type="text" name="first_name" value="{{ old('first_name') }}" placeholder="مثال: سارة" style="width:100%; padding:10px 14px; border:1.5px solid rgba(0,83,122,0.14); border-radius:10px; background:#FBFEFF; color:#013C58; font-size:13px; font-family:'Tajawal',sans-serif; outline:none;">
            </div>
            <div>
                <label style="display:block; font-size:12.5px; font-weight:700; color:#013C58; margin-bottom:6px;">الاسم الأخير</label>
                <input type="text" name="last_name" value="{{ old('last_name') }}" placeholder="مثال: يوسف" style="width:100%; padding:10px 14px; border:1.5px solid rgba(0,83,122,0.14); border-radius:10px; background:#FBFEFF; color:#013C58; font-size:13px; font-family:'Tajawal',sans-serif; outline:none;">
            </div>
        </div>

        <div style="margin-bottom:16px;">
            <label style="display:block; font-size:12.5px; font-weight:700; color:#013C58; margin-bottom:6px;">البريد الإلكتروني</label>
            <input type="email" name="email" value="{{ old('email') }}" placeholder="admin@example.com" style="width:100%; padding:10px 14px; border:1.5px solid rgba(0,83,122,0.14); border-radius:10px; background:#FBFEFF; color:#013C58; font-size:13px; font-family:'Tajawal',sans-serif; outline:none;">
        </div>
        ...
        <div style="margin-bottom:22px;">
            ...
            <p style="margin:6px 0 0; font-size:11.5px; color:rgba(1,60,88,0.45);">رح تنبعت هالكلمة تلقائيًا للأدمن عبر إيميل ترحيبي. بعد الحفظ رح تروحي مباشرة لصفحة تحديد صلاحياته.</p>
        </div>

        <div style="display:flex; justify-content:flex-end; gap:10px;">
            <a href="{{ route('admin.permission.index') }}" style="display:inline-flex; align-items:center; padding:10px 20px; border-radius:10px; background:rgba(0,83,122,0.08); color:#00537A; text-decoration:none; font-family:'Poppins',sans-serif; font-weight:600; font-size:12.5px;">إلغاء</a>
