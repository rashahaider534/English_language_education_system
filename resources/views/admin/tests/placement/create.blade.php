...
@endphp
<div
    x-data="placementTestForm(@js($questionsForJs))"
...
>
    ...
    <form action="{{ route('tests.placement.placement.store') }}" method="POST" @submit="beforeSubmit">
        @csrf
        ...
        <div class="t-panel" style="background:#EFFAFD; border:1.5px solid rgba(0,83,122,0.16); border-radius:20px; padding:26px; margin-bottom:22px; box-shadow:0 10px 26px rgba(0,83,122,0.06);">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                <div>
                    ...
                    <div class="t-field-wrap"><input type="text" name="title_en" placeholder="e.g. Placement Test 1" required></div>
                </div>
                <div>
                    ...
                    <div class="t-field-wrap"><input type="text" name="title_ar" placeholder="مثال: اختبار تحديد المستوى 1" required></div>
                </div>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:22px; align-items:start;">
            ...
            <div class="t-panel" style="background:#EFFAFD; border:1.5px solid rgba(14,106,150,0.35); border-radius:20px; padding:24px; box-shadow:0 10px 26px rgba(0,83,122,0.06);">
                ...
                <p style="margin:0 0 14px; font-size:11.5px; color:rgba(1,60,88,0.5);">
                    عدد الأسئلة: <span x-text="selected.length" style="font-weight:700;"></span> (لازم إجمالي إجابتين على الأقل). رتبيهن بالأسهم.
                </p>
                    ...
                    <template x-for="(s, i) in selected" :key="s.id">
                        <div class="t-selected-row" style="display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:11px; background:rgba(255,211,91,0.08);">
                            ...
                            <div style="display:flex; flex-direction:column; gap:2px;">
                                <button type="button" class="t-mini-btn" @click="moveUp(i)" :disabled="i === 0" style="width:22px; height:18px; border:none; border-radius:5px; background:rgba(0,83,122,0.08); color:#00537A; cursor:pointer; opacity:1;" :style="i === 0 ? 'opacity:0.3; cursor:not-allowed;' : ''">▲</button>
                                <button type="button" class="t-mini-btn" @click="moveDown(i)" :disabled="i === selected.length - 1" style="width:22px; height:18px; border:none; border-radius:5px; background:rgba(0,83,122,0.08); color:#00537A; cursor:pointer;" :style="i === selected.length - 1 ? 'opacity:0.3; cursor:not-allowed;' : ''">▼</button>
                            </div>
...
<script>
function placementTestForm(pool) {
    return {
        pool: pool,
        selected: [],
        search: '',
