...
>
    ...
    <div style="background:linear-gradient(135deg,#013C58 0%, #00537A 60%, #0E6A96 130%); border-radius:22px; padding:26px 32px; margin-bottom:22px; box-shadow:0 20px 44px rgba(1,60,88,0.2);">
        <h1 style="margin:0; font-family:'Poppins',sans-serif; font-weight:800; font-size:22px; color:#fff;">اختبار جديد لمستوى: {{ $level->name_ar ?? $level->name_en }}</h1>
        <p style="margin:6px 0 0; font-size:13px; color:rgba(168,232,249,0.8);">حددي عدد الأسئلة لكل درجة صعوبة، والنظام رح يختارهن تلقائيًا من أسئلة كورسات هالمستوى.</p>
    </div>
    ...
    <form action="{{ route('tests.level.levelTest.generate', $level) }}" method="POST">
        ...
        <div class="t-panel" style="background:#EFFAFD; border:1.5px solid rgba(0,83,122,0.16); border-radius:20px; padding:26px; margin-bottom:22px; box-shadow:0 10px 26px rgba(0,83,122,0.06);">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
                <div>
                    ...
                    <div class="t-field-wrap"><input type="text" name="title_en" value="{{ old('title_en') }}" placeholder="e.g. Level 1 Final Test" required></div>
                </div>
                <div>
                    ...
                    <div class="t-field-wrap"><input type="text" name="title_ar" value="{{ old('title_ar') }}" placeholder="مثال: اختبار نهاية المستوى الأول" required></div>
                </div>
            </div>
        ...
        <button type="submit" class="t-submit-btn" :disabled="totalCount === 0" :style="totalCount === 0 ? 'opacity:0.45; cursor:not-allowed;' : 'cursor:pointer;'" style="display:inline-flex; align-items:center; gap:8px; padding:13px 30px; border-radius:12px; border:none; background:linear-gradient(90deg,#F5A201,#FFBA42); color:#013C58; font-family:'Poppins',sans-serif; font-weight:700; font-size:14px;">
            إنشاء الاختبار
        </button>
    </form>
</div>
