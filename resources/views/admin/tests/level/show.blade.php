                ...
                @if ($showPublishButton)
                    @if ($canPublish)
                        <form @submit.prevent="submitGuarded($event, 'لا تملك صلاحية كافية لنشر الاختبار')" action="{{ route('admin.content-review.tests.publish', $test) }}" method="POST">
                            @csrf
                            <button type="submit" style="display:inline-flex; align-items:center; gap:8px; padding:12px 20px; border-radius:12px; border:none; background:rgba(255,255,255,0.14); color:#fff; font-family:'Poppins',sans-serif; font-weight:700; font-size:13px; cursor:pointer; flex-shrink:0;">
