...
<div class="-mx-4 -my-6 px-4 py-6 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8" style="background:#DFF2F9; font-family:'Tajawal',sans-serif; min-height:100vh;" dir="rtl">
    ...
    <div style="margin-bottom:18px;">
        <a href="{{ route('lessons.show', $lesson) }}{{ $backFromReview ? '?from=review' : '' }}" style="display:inline-flex; align-items:center; gap:6px; color:#00537A; font-size:13px; font-weight:600; text-decoration:none;">
    ...
    <div class="t-panel" style="background:#EFFAFD; border:1.5px solid rgba(0,83,122,0.16); border-radius:22px; overflow:hidden; box-shadow:0 18px 44px rgba(0,83,122,0.06);">
        @forelse ($testVersions as $test)
            ...
            @endphp
            <a href="{{ route('test.show', $test) }}{{ $backFromReview ? '?from=review' : '' }}" class="t-row" style="display:flex; align-items:center; gap:16px; padding:18px 24px; border-bottom:1px solid rgba(0,83,122,0.06); text-decoration:none;">
