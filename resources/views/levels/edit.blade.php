...
@php
    ...
    $isPublished = $level->status === 'published';
    $isLocked = in_array($level->status, ['closed', 'archived']);
    $isNameLocked = $isLocked;
    $isCoreLocked = $isPublished || $isLocked; // order/min/max/price
    $isDurationLocked = $isLocked; // duration stays editable when published
...
<div style="font-family:'Tajawal',sans-serif; max-width:680px; margin:0 auto;" dir="rtl">
    ...
    <form method="POST" action="{{ route('levels.update', $level) }}" style="background:#fff; border:1px solid rgba(0,83,122,0.08); border-radius:22px; padding:28px 26px; box-shadow:0 18px 44px rgba(0,83,122,0.06);">
        @csrf
        @method('PUT')
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
            <div>
                ...
                <div style="{{ $isNameLocked ? $wrapLocked : $wrapBase }}">
                    <input name="name_en" value="{{ old('name_en', $level->name_en) }}" style="{{ $input }}" @if($isNameLocked) disabled @endif>
                </div>
            </div>
            <div>
                <label style="{{ $label }}">بالعربي</label>
                <div style="{{ $isNameLocked ? $wrapLocked : $wrapBase }}">
                    <input name="name_ar" value="{{ old('name_ar', $level->name_ar) }}" style="{{ $input }}" @if($isNameLocked) disabled @endif>
                </div>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px;">
            <div>
                ...
                <div style="{{ $isCoreLocked ? $wrapLocked : $wrapBase }}">
                    <input type="number" name="order" value="{{ old('order', $level->order) }}" style="{{ $input }}" @if($isCoreLocked) disabled @endif>
                </div>
            </div>
            <div>
                <label style="{{ $label }}">أدنى علامة</label>
                <div style="{{ $isCoreLocked ? $wrapLocked : $wrapBase }}">
                    <input type="number" name="minimum_score" value="{{ old('minimum_score', $level->minimum_score) }}" style="{{ $input }}" @if($isCoreLocked) disabled @endif>
                </div>
            </div>
            <div>
                <label style="{{ $label }}">أعلى علامة</label>
                <div style="{{ $isCoreLocked ? $wrapLocked : $wrapBase }}">
                    <input type="number" name="maximum_score" value="{{ old('maximum_score', $level->maximum_score) }}" style="{{ $input }}" @if($isCoreLocked) disabled @endif>
                </div>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
            <div>
                ...
                <div style="{{ $isCoreLocked ? $wrapLocked : $wrapBase }}">
                    <input type="number" name="price" value="{{ old('price', $level->price) }}" style="{{ $input }}" @if($isCoreLocked) disabled @endif>
                </div>
            </div>
            <div>
                <label style="{{ $label }}">المدة المتوقعة (أسابيع)</label>
                <div style="{{ $isDurationLocked ? $wrapLocked : $wrapBase }}">
                    <input type="number" name="estimated_duration" value="{{ old('estimated_duration', $level->estimated_duration) }}" style="{{ $input }}" @if($isDurationLocked) disabled @endif>
                </div>
            </div>
        </div>
